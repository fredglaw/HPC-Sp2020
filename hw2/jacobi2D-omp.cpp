...
// g++ -std=c++11 -O3 jacobi2D-omp.cpp && ./a.out -N 100
#include <stdio.h>
#include <stdlib.h>
#include <cmath>
#include <omp.h>
...
double jacobi(double* u_curr, double* u_prev, double init_resid, double h_sq,
...
int main(int argc,char *argv[])
{
  ...
  for (long num_times=0; num_times < N_REPEATS; num_times++){
    ...
    t.tic();
    jacobi(u_curr, u_prev, init_resid,h_sq,N,Np2,Nmax, resid_terminate);
    time += t.toc();
  }
  printf("%d Jacobi finished, avg time = %f\n",N_REPEATS,time/(double)N_REPEATS);
