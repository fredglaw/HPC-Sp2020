...
void print_results(float array[N], int tid, int section)
{
  ...
  // removed a #pragma omp barrier in print_results. This was causing
  // all the threads to wait which led to runtime error. Removal of this led to
  // two threads executing the two sections and then all threads joining.
  // #pragma omp barrier
